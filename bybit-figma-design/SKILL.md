...
// Font loader (handles SemiBold naming)
async function loadFont(family, style) {
  try { await figma.loadFontAsync({ family, style }); }
  catch { await figma.loadFontAsync({ family, style: style.replace('SemiBold', 'Semi Bold') }); }
}
...
// Override component text (must set fontName before characters)
async function overrideText(instance, searchText, newText) {
  const textNodes = instance.findAll(n => n.type === 'TEXT');
  for (const t of textNodes) {
    if (t.characters.toLowerCase().includes(searchText.toLowerCase())) {
      const fn = t.fontName;
      await loadFont(fn.family, fn.style);
      t.fontName = { family: fn.family, style: fn.style.replace('SemiBold', 'Semi Bold') };
...
// Text style IDs (Typography2025)
const TS = {
  'Semibold/24': 'S:63a28af908557280e4a282f03952ab43729013e5,2609:14',
  'Semibold/20': 'S:35fb29567075d8502fc1f847e70a5db565d94d47,2609:13',
  'Semibold/18': 'S:c263f6c79f9e992bc6d3b1e0762fea8253720a0e,2609:12',
  'Semibold/16': 'S:fb6b05c3ebc979b4616801a01aea9b2991a281c7,2609:11',
  'Semibold/14': 'S:95c43064b7107a35a1c85e28bca7fd870c03dda7,2609:10',
  'Semibold/12': 'S:ad093050bf6f479e6c382e0317c64bac1bb428af,2609:9',
  'Semibold/10': 'S:7f31e3a76bcf8dd27330216dbc054c6602dcbd15,2609:8',
  'Regular/16':  'S:eef16a67cb4eba4134ab99e4bdd56bf24c937d5c,2609:3',
  'Regular/14':  'S:9e2e5ce326940a39f1a397744f6b126b62c01512,2609:2',
  'Regular/12':  'S:8d1cf8fd6e5f75224901553b25d4f1d075e654e7,2609:1',
  'Regular/10':  'S:b271c0aacc59f19ad17fe6ec5252470c211ea2b4,2609:0',
};
function bindStyle(textNode, name) { if (TS[name]) textNode.textStyleId = TS[name]; }
...
- `bindStyle(text, 'Semibold/20')` — 绑定文字样式
...
**使用库文字样式 ID 绑定，而非只设 fontSize/fontWeight。** Standard Helpers 的 `TS` map + `bindStyle` 已包含 Typography2025 全部样式。

```js
bindStyle(titleNode, 'Semibold/20');
bindStyle(bodyNode, 'Regular/14');
```
...
| Font "Inter SemiBold" 加载失败 | Figma 用 "SemiBold" 但 API 要 "Semi Bold" | `style.replace('SemiBold', 'Semi Bold')` |
...
| USDT Logo | `4a57f00e16c48e75f0535b978fc24e4a0c6bd2c0` | Tether |
| USDC Logo | `03d2cd88d7a35e58660a43cbaa7b990e4e40e02c` | USD Coin |
| **Controls (高频)** | | |
